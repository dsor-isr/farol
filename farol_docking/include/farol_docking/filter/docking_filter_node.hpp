...
class DockingFilterNode {
 ...
 private:
  ...
  // Publishers
  ros::Publisher state_pub_, body_velocity_pub_, usbl_meas_dock_pub_, usbl_meas_auv_pub_;
  ...
  geometry_msgs::Vector3 vec3_msg_;
  
 	// ROS node iteration timer
 	ros::Timer timer_;    

  // ROS parameters
  double node_frequency_, outage_bearing_dock_, outage_elevation_auv_; 
  ...
  bool realistify_;
  double current_range_; // for dvl outlier sim
};
