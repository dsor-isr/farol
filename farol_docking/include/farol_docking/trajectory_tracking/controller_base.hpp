...
public:
  ...
  // Life‑cycle hooks (optional in derived)
  ...
  // === Reference (world frame unless noted) ===
  Eigen::Vector3d p_d_{Eigen::Vector3d::Zero()}; // position
  Eigen::Vector3d pd_d_{Eigen::Vector3d::Zero()}; // velocity
  Eigen::Vector3d pdd_d_{Eigen::Vector3d::Zero()}; // acceleration


  Eigen::Matrix3d R_d_{Eigen::Matrix3d::Identity()}; // attitude
  Eigen::Vector3d w_d_{Eigen::Vector3d::Zero()}; // desired body rates
  Eigen::Vector3d wdd_d_{Eigen::Vector3d::Zero()}; // desired body accels


  // === Current state (body rates/vel preferred) ===
